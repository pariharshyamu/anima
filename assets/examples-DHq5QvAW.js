const e=`import { Game } from 'gama3d';
...
scene.add(ground);`,t=[{id:"gallery",title:"Every character is a seed",group:"Characters",code:`// Five strangers from five integers: bodies, figures, outfits, faces,
...
game.start();`},{id:"creator",title:"The creator API",group:"Characters",code:`// describeHumanoid resolves EVERY seeded decision into a plain spec.
...
game.camera.lookAt(0, 1.05, 0);
game.onUpdate((t) => cast.forEach((loco) => loco.update(t.delta, 0)));
game.start();`},{id:"faces",title:"Faces & expressions",group:"Characters",code:`// One person, five moods: the brow angle and mouth corners ARE the
...
game.onUpdate((t) => cast.forEach((loco) => loco.update(t.delta, 0)));
game.start();`},{id:"wardrobe",title:"Wardrobe & body types",group:"Characters",code:`// Garment layers, not painted-on colors: dress, top+skirt, jacket,
...
game.onUpdate((t) => cast.forEach((loco) => loco.update(t.delta, 0)));
game.start();`},{id:"locomotion",title:"Idle → walk → run",group:"Animation",code:`// The 1D blend: velocity in, correct gait out — phase-synced, with
...
game.start();`},{id:"craft",title:"Foot IK, gaze & a wave",group:"Animation",code:`// The craft layer on a SCENA hillside: feet plant on the actual
...
game.start();`},{id:"sockets",title:"Sockets: carry things",group:"Animation",code:`// Props ride bones through every animation: a torch attached to the
...
game.start();`},{id:"crowd",title:"A VAT crowd on a road",group:"Scale",code:`// Sixty villagers as three draw calls: bodies baked into Vertex
...
game.start();`},{id:"trio",title:"The trio, together",group:"Scale",code:`// The whole family in one scene: SCENA terrain/road/forest, GAMA
...
game.start();`},{id:"village",title:"Havenbrook: a medieval village",group:"Scale",code:`// The whole trilogy in one scene. SCENA builds the world and every prop —
...
game.start();`},{id:"race",title:"Pocket racer (playable!)",group:"Games",code:`// A playable racer — phone AND desktop — where the whole game (player car,
...
game.start();`},{id:"mechanisms",title:"Manipulables (operate the world)",group:"Games",code:`// Props with STATE that animate when operated: a keeper throws a lever
...
game.start();`},{id:"carry",title:"Carryables (pick up · carry · throw)",group:"Games",code:`// The carry verb: a porter picks up a crate, carries it (still walking),
...
game.start();`},{id:"labor",title:"Work stations (chop · mine · saw · stir)",group:"Games",code:`// Rhythmic work: the worker chops, mines, saws and stirs — each a looping
...
const camTarget = new Vector3(5, 5, 12);
game.camera.position.set(5, 5, 12);
game.start();`},{id:"miami",title:"Miami beach walk (playable!)",group:"Games",code:`// A PLAYABLE HUMANOID ON A SCENA BEACH — phone and desktop, one code
...
game.start();`},{id:"cricket",title:"Two-over cricket (playable!)",group:"Games",code:`// A PLAYABLE TWO-OVER MATCH — phone and desktop, one code path. You bat.
...
game.start();`},{id:"club",title:"The club (web radio · DJ tiles · dance styles)",group:"Games",code:`// THE THREE-WAY COMPOSITION: SCENA owns the woofer and the DJ tiles,
...
game.start();`},{id:"yoga",title:"Yoga (asanas · breath · the held frame)",group:"Animation",code:`// YOGA is the anti-dance: the clock is BREATH (a sine, not a tick), and
...
game.start();`},{id:"gathering",title:"Gatherings (choosing a seat · sitting down · talking)",group:"Games",code:`// Several people to one prop — and the behaviour that makes them read as
...
game.start();`},{id:"rowing",title:"A crew, and one number",group:"Animation",code:`// SCENA's oar bank works out the stroke and publishes phaseAt(seat).
...
game.start();`},{id:"riding",title:"Riding a horse (mount · gaits · ladder)",group:"Games",code:`// A horse, built as a real quadruped: the skeleton is laid out to horse
...
game.start();`},{id:"screens",title:"Screens (a room lit by television)",group:"Scale",code:`// A flat after dark. Nothing in this room emits light except screens.
...
game.start();`},{id:"phone",title:"On the phone (pose, not prop)",group:"Characters",code:`// The handset is a few pixels across at this distance, so the POSE does
...
game.start();`},{id:"queue",title:"Queueing at a cash machine",group:"Scale",code:`// SCENA says where the line is; GAMA says who is where along it. Waiting
...
game.start();`},{id:"office",title:"Smart office (wiring · desk work)",group:"Scale",code:`// Two things at once. The desk poses are ANIMA's: typing is carried by the
...
game.camera.position.set(2.0, 1.9, 1.7);
game.start();`},{id:"knockout",title:"Dodgeball: hits, KO & the get-up",group:"Games",code:`// PHASE C OF THE GAMING ROADMAP, all three libraries in one loop.
...
game.start();`},{id:"lamplighter",title:"The lamplighter",group:"Scale",code:`// THE LIGHTING ARC'S PAYOFF. Dusk falls on a village lane (SCENA's
...
game.start();`},{id:"sortie",title:"The sortie",group:"Scale",code:`// THE SORTIE — the aviation arc's finale, and the trilogy's handshake in
...
game.start();`},{id:"parkour",title:"Parkour: two bodies, one course",group:"Scale",code:`// TWO BODIES, ONE COURSE — and they do not agree about it.
//
...
game.start();`},{id:"dogfight",title:"Dogfight: fly it, and count the rounds",group:"Scale",code:`// A DOGFIGHT YOU FLY, and the whole trilogy under it.
//
...
game.start();`},{id:"mood",title:"Mood: one walk, thirteen ways to carry it",group:"Scale",code:`// THIRTEEN IDENTICAL BODIES, ONE WALK CYCLE, THIRTEEN MOODS.
//
...
game.start();`},{id:"gym",title:"The gym: eight reps, and the eighth is not the first",group:"Scale",code:`// TWELVE LIFTERS, TWELVE MOVEMENTS, AND A LINE BEHIND EVERY BAR.
//
...
game.start();
`},{id:"dining",title:"The table: eight utensils, one body",group:"Scale",code:`// EIGHT DINERS, EIGHT UTENSILS, ONE BODY.
//
...
game.start();
`},{id:"archery",title:"The range: five bows, and the anchor decides the group",group:"Scale",code:`// FIVE ARCHERS, FIVE BOWS, AND THE ARROWS ACTUALLY FLY.
//
...
game.start();
`},{id:"striking",title:"The bag room: the mass is the body behind it",group:"Scale",code:`// SIX FIGHTERS, FOURTEEN STRIKES, AND NOBODY TYPED A DAMAGE NUMBER.
//
...
game.start();
`},{id:"sparring",title:"Sparring: what gets through is geometry",group:"Scale",code:`// SEVEN GUARDS, ONE ATTACKER, AND NO BLOCK CHANCE ANYWHERE.
//
...
game.start();
`},{id:"dojo",title:"Dojo: no kuzushi, no throw",group:"Scale",code:`// FIVE PAIRS, THE SAME THROW, AND NOTHING BUT THE PULL BETWEEN THEM.
//
...
game.start();
`},{id:"styles",title:"Styles: a style is where the feet are",group:"Scale",code:`// SIX FIGHTERS, SIX STYLES, AND NOT ONE DAMAGE MULTIPLIER BETWEEN THEM.
//
...
game.start();
`},{id:"bout",title:"Bout: nobody encoded the reach advantage",group:"Scale",code:`// TWO FIGHTERS. THE AI READS FOUR NUMBERS AND NOT ONE OF THEM IS HEIGHT.
//
...
game.start();
`},{id:"armoury",title:"Armoury: nine objects, and not one of them was weighed",group:"Scale",code:`// NINE WEAPONS HANGING FROM THEIR OWN GRIPS, RELEASED TOGETHER.
//
...
game.start();
`},{id:"proving",title:"The proving ground: sharpness starts it, toughness pays for it",group:"Scale",code:`// FIVE EDGES OF THE SAME STEEL, ONE PUSH, AND A LINE THEY EITHER CLEAR OR DO NOT.
//
...
game.start();
`},{id:"crossing",title:"The crossing: shallow grips and will not hold still",group:"Scale",code:`// TWO BLADES IN CONTACT STOP BEING TWO OBJECTS.
//
...
game.start();
`},{id:"thrown",title:"Thrown: the four centimetres that ended the hundred-metre javelin",group:"Scale",code:`// TWO JAVELINS, RELEASED IDENTICALLY, AND THE ONLY DIFFERENCE IS 4 cm OF BALANCE.
//
...
game.start();
`},{id:"fence",title:"The bout: two fencers, and they do not stand still",group:"Scale",code:`// AN ARMED BOUT, AND THE FEET ARE HALF OF IT.
//
...
game.start();
`},{id:"aloud",title:"Aloud: the browser speaks, ANIMA moves the mouth",group:"Character",code:`// THE SEAM, WITH A REAL VOICE ON THE OTHER SIDE OF IT.
//
...
game.start();
`},{id:"talking",title:"Speech: two mouths, and only one of them shuts",group:"Character",code:`// THE VISEME TABLE HAS BEEN PUBLISHED SINCE 1888.
//
...
game.start();
`},{id:"matching",title:"Motion matching: the weights were units all along",group:"Character",code:`// A CONTROLLER THAT IS A SEARCH, NOT A BLEND TREE.
//
...
game.start();
`},{id:"floor",title:"The floor: gaze is how a turn changes hands",group:"Character",code:`// TWO PEOPLE, AND A THIRD NUMBER NEITHER OF THEM WAS GIVEN.
//
...
game.start();
`}];function s(a){return t.find(o=>o.id===a)??t[0]}export{t as E,s as f};
